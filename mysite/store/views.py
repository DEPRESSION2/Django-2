...
class PurchaseCreateView(LoginRequiredMixin,CreateView):
    ...
    def form_valid(self, form):
        ...
        product = Product.objects.get(id=self.request.POST['product_id'],user=self.request.user)
        ...
        if amount_of_money > object.user.wallet:
            ...
            messages.error(self.request, 'Товара нету на складе')
            return redirect('/')
...
class PurchaseReturnCreateView(LoginRequiredMixin,CreateView):
    ...
    def form_valid(self, form):
        self.object = form.save(commit=False)
        purchase = Purchase.objects.get(id=self.request.POST['purchase_id'],user=self.request.user)
        returns = PurchaseReturns.objects.filter(purchase=purchase)
        if purchase.time + timedelta(minutes=3) < timezone.now():
            messages.error(self.request, 'Сорян но ты уже не сможешь вернуть его')
            return redirect('purchase')
        elif returns:
            messages.info(self.request, 'Запрос на возрат уже на расмотрении')
            return redirect('purchase')
        messages.info(self.request, 'Запрос на расмотрении')
